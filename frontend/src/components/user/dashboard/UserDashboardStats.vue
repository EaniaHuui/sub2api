    ...
    <div class="card p-4">
      <div class="flex items-center gap-3">
        ...
        <div>
          ...
          <p class="text-xl font-bold text-purple-600 dark:text-purple-400">${{ formatCost(stats?.today_actual_cost || 0) }}</p>
          <p class="text-xs text-gray-500 dark:text-gray-400">{{ t('common.total') }}: ${{ formatCost(stats?.total_actual_cost || 0) }}</p>
        </div>
      </div>
    </div>
  </div>
